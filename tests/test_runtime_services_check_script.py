...
def test_runtime_services_check_script_has_required_defaults() -> None:
    ...
    assert 'RUNTIME_API_READINESS_URL' in script
    assert 'RUNTIME_ADMIN_URL' in script
    assert 'RUNTIME_ADMIN_DB_READY_URL' in script
...
def test_deploy_script_runs_runtime_services_check() -> None:
    ...
    assert 'RUNTIME_API_READINESS_URL' in deploy_script
