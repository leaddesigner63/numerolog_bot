...
- Проверьте, что worker не создаёт warning-шторм при временных ошибках отправки nudge: в `screen_state.data` должны появляться `resume_nudge_error_count` / `resume_nudge_next_retry_at` (или `checkout_value_nudge_*`), а число предупреждений в логе не растёт на каждом цикле до наступления `next_retry_at`.
...
21. Для быстрого hot-switch оплаты используйте связку env:
   - `PAYMENT_MODE=provider` — штатная работа через провайдера;
   - `PAYMENT_MODE=manual` + `MANUAL_PAYMENT_CARD_NUMBER=<реквизиты>` — временный ручной режим;
   - возврат в штатный режим: снова `PAYMENT_MODE=provider` (без изменения кода).
   - пример: `provider -> manual -> provider` описан в `docs/deploy/autodeploy_manual_payment_mode_step_by_step.md`.
22. Для доступа к веб-админке задайте `ADMIN_LOGIN` и `ADMIN_PASSWORD` и при необходимости ограничьте доступ к `/admin` по сети (фаервол).
23. Для фонового воркера отчётов можно настроить интервалы опроса и таймаут блокировки:
   - `REPORT_JOB_POLL_INTERVAL_SECONDS` (по умолчанию 5)
   - `REPORT_JOB_LOCK_TIMEOUT_SECONDS` (по умолчанию 600)
24. Если вы используете несколько сервисов, решите: будете ли перезапускать их списком (`SERVICE_NAMES`) или через общий `target` (например, `numerolog.target`).
   Если сервисов нет или имена не совпадают — в деплое будет ошибка, поэтому сначала создайте unit-файлы.
25. Для мониторинга критических сбоев генерации отчёта можно указать `MONITORING_WEBHOOK_URL` (бот отправит событие `report_generate_failed`).
