...
7. Для Prodamus укажите `PRODAMUS_STATUS_URL` (эндпоинт проверки статуса платежа по order_id), `PRODAMUS_SECRET` и `PRODAMUS_WEBHOOK_SECRET`.
8. Для CloudPayments укажите `CLOUDPAYMENTS_PUBLIC_ID` и `CLOUDPAYMENTS_API_SECRET`, иначе fallback не будет работать.
9. Убедитесь, что в `.env` добавлены ключи LLM: `GEMINI_API_KEY`/`GEMINI_MODEL` и `OPENAI_API_KEY`/`OPENAI_MODEL` (fallback).
