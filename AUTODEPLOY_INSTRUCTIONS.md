...
13. Если нужно скрыть технические названия экранов (префикс `S1:`), задайте `SCREEN_TITLE_ENABLED=false`.
14. Если хотите хранить картинки экранов вне репозитория, задайте `SCREEN_IMAGES_DIR` (например, `/var/www/numerolog_bot/storage/screen_images`).
15. Если нужно включить искусственную задержку перед выдачей отчёта, задайте `REPORT_DELAY_SECONDS` в секундах.
16. Если нужно отключить post-фильтрацию отчёта (без проверки контентной безопасности), добавьте `REPORT_SAFETY_ENABLED=false`.
17. Если нужно отключить проверку оплаты (например, для тестового запуска), добавьте `PAYMENT_ENABLED=false`.
18. Для доступа к веб-админке задайте `ADMIN_API_KEY` и ограничьте доступ к `/admin` по сети (фаервол/allowlist).
19. Если вы используете несколько сервисов, решите: будете ли перезапускать их списком (`SERVICE_NAMES`) или через общий `target` (например, `numerolog.target`).
   Если сервисов нет или имена не совпадают — в деплое будет ошибка, поэтому сначала создайте unit-файлы.
20. Для мониторинга критических сбоев генерации отчёта можно указать `MONITORING_WEBHOOK_URL` (бот отправит событие `report_generate_failed`).
