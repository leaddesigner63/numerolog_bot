...
5. В отчёте (S7/S13) и в личном кабинете (S11) доступна кнопка **«Сообщество»**, ведущая в канал проекта (настраивается через `COMMUNITY_CHANNEL_URL`). В личном кабинете также есть кнопка **«Обратная связь»** для перехода на экран отправки сообщения в группу.
...
Для доступа нужен ключ `ADMIN_API_KEY` из `.env`: при открытии страницы появится экран ввода ключа, после чего админка откроется и сохранит ключ в cookie и localStorage для последующих запросов. Если в `.env` значение обёрнуто в кавычки, сервер автоматически уберёт их при чтении, поэтому вводите ключ без кавычек.  
Если хотите сразу открыть админку, можно перейти по адресу `http://<host>:<port>/admin?key=<ADMIN_API_KEY>` — сервер сохранит ключ в cookie и перенаправит на `/admin` без параметров, чтобы ключ не оставался в адресной строке.  
...
- сообщения обратной связи отображаются полностью с сохранением переносов строк (контент экранируется в интерфейсе, данные в БД сохраняются как есть);
...
> Важно: админка не предназначена для публичного доступа — обязательно используйте сложный `ADMIN_API_KEY` и при необходимости ограничьте доступ по сети/фаерволу.
