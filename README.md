...
### Прод-архитектура запуска (обязательно)

В production должны одновременно работать **ровно 2 процесса**:
...
2. Для T1–T3 сначала откройте описание тарифа (S2) — на этом шаге цена и кнопка оплаты не показываются. Затем заполните экран **Мои данные** (S4). Доступ к заполнению профиля открыт до оплаты. Пол выбирается кнопками «Женский»/«Мужской», дата, время и место рождения принимаются в любом формате.
   После сохранённого профиля в сценарии `profile_flow=True` экран «Мои данные» показывает только две кнопки: **«Редактировать»** и **«Продолжить»**.
   Если профиль уже заполнен, кнопка **«Продолжить»** также остаётся доступной и для T0-сценария (бесплатный отчёт), чтобы перейти к следующему шагу без повторного редактирования данных.
   Если профиль уже заполнен в checkout-флоу (T1/T2/T3), кнопки **«Удалить мои данные»** и **«Кабинет»** на S4 скрываются, чтобы не уводить пользователя из создания заказа.
3. Для T2/T3 после профиля обязательно проходится расширенная анкета (S5) — она также доступна до оплаты. Ответы принимаются в произвольном формате без форматирования и валидации, сохраняются после каждого шага и доступны для продолжения.
4. Экран оплаты для T1/T2/T3 теперь финальный: он показывается только после завершения данных (и анкеты для T2/T3), непосредственно перед генерацией отчёта (S3 -> S6 -> S7). На S3 отображаются название тарифа, цена, компактный блок «что получите / когда получите», основная кнопка оплаты и вторичная кнопка «Что входит в отчёт» (без выхода из checkout-флоу). Заказ и платёжная ссылка создаются только на этом финальном шаге checkout. Для T0 поведение не изменено.
   После подтверждения оплаты запускается генерация отчёта — перед выдачей может применяться искусственная задержка с анимацией и таймером (настраивается через `REPORT_DELAY_SECONDS`), затем PDF формируется автоматически и отправляется в чат. При сбоях генерации или доставки бот вежливо извиняется и повторяет запрос, чтобы пользователь всё равно получил результат.
   Если пользователь возвращается в бот по payload `paywait_<order_id>`, для неоплаченного заказа он попадает в финальное ожидание оплаты (S3), а для оплаченного — сразу в post-payment шаг генерации (S6/S7 в зависимости от статуса report job).
...
### Checkout-at-end: эталонный порядок шагов

- **T1**: `S1 -> S2 -> S4 -> S3 -> S6 -> S7`
- **T2/T3**: `S1 -> S2 -> S4 -> S5 -> S3 -> S6 -> S7`
- **T0**: без оплаты, текущий совместимый флоу сохранён

### Актуализация S2 (первый экран тарифа)

- На первом тарифном экране теперь показываются только 3 коротких буллета: **результат / польза / для кого**.
- Длинный продающий текст вынесен в отдельный шаг **S2_MORE** с кнопками `Назад` и `Продолжить`.
- На S2 добавлены CTA-кнопки: `Продолжить` (основной путь) и `Подробнее` (вторичный путь).
- В аналитике переходов используются отдельные callback-триггеры `s2:details`, `s2:details:back`, `s2:details:continue` для оценки влияния шага «Подробнее» на конверсию в S4/S3.
- Для checkout-шага S3 добавлен quality-signal `s3:report_details`: в finance-summary считаются отдельные метрики кликов и уникальных пользователей, открывших факт-лист «Что входит в отчёт».

### Анти-зацикливание post-payment экранов

- При повторном открытии экрана оплаты `S3` для уже оплаченного заказа бот сразу переводит пользователя в следующий шаг (`S4` или `S5` для T2/T3 с незавершённой анкетой), чтобы исключить бесконечное переключение между экранами оплаты и профиля.
...
- Целевой smoke-check нового флоу checkout-at-end:
  - `bash scripts/smoke_check_checkout_flow.sh`
- Релизный чеклист checkout-at-end:
  - `docs/deploy/checkout_flow_release_checklist.md`
- Прод-деплой через GitHub Actions описан в `AUTODEPLOY_INSTRUCTIONS.md`.

### Регрессионное покрытие флоу оплаты

- Цена не отображается на ранних шагах (S2) и отображается только на финальном checkout (S3).
- Профиль (S4) и анкета T2/T3 (S5) доступны до оплаты.
- Для платных тарифов запуск генерации без `paid` блокируется.
- После `paid` deep-link `paywait_<order_id>` переводит в post-payment генерацию (S6/S7).
...
- Полный список обновлённых каталогов и сценариев, где нужно заменить изображения после обновления `description.txt`, находится в `docs/screen_images_update_matrix.md`.
...
- в сводке добавлен виджет health генератора отчётов: online/offline, возраст heartbeat, pending/in_progress задач и warning-баннер «Оплаты есть, генерация может быть недоступна» при offline worker;
...
- После успешной генерации отчёт сохраняется в таблице `reports`: `report_text` хранит сырой ответ провайдера (для аудита/отладки), а `report_text_canonical` — итоговый очищенный текст для пользовательских экранов и PDF.
...
- После генерации отчёта создаётся запись в `reports`; для платных тарифов отчёт связывается с оплаченной записью `orders`, а в `report_text_canonical` фиксируется единая финальная версия текста для повторных показов и выгрузок.
...
- `PAYMENT_PROVIDER`, `PRODAMUS_FORM_URL`, `PRODAMUS_KEY` (или legacy: `PRODAMUS_API_KEY`/`PRODAMUS_SECRET`/`PRODAMUS_WEBHOOK_SECRET`),
  ...
  `PAYMENT_WEBHOOK_URL`, `PAYMENT_SUCCESS_URL`, `PAYMENT_FAIL_URL`, `PAYMENT_PROVIDER_POLL_MIN_INTERVAL_SECONDS`, `TELEGRAM_BOT_USERNAME`
...
- Тариф-зависимые экраны: `S2_T0`, `S2_T1`, `S2_T2`, `S2_T3` (аналогично для `S3`, `S5`, `S6`, `S7`).
- Для `S4` поддерживаются сценарные каталоги: `S4_PROFILE_T0`...`S4_PROFILE_T3` (обычный вход в ЛК) и `S4_AFTER_PAYMENT_T0`...`S4_AFTER_PAYMENT_T3` (вход в профиль сразу после оплаты). Также доступны fallback-каталоги `S4_PROFILE`, `S4_AFTER_PAYMENT`, `S4_T0`...`S4_T3` и `S4`.
...
- На экране `S4_CONSENT` кнопка `✅ Подтвердить` фиксирует обязательное согласие и согласие на уведомления, а кнопка `📩 Отказ от уведомлений` (`profile:consent:accept_without_marketing`) фиксирует обязательное согласие и отказ от рассылки.
...
## Обновление checkout (paid -> продолжение без повторной оплаты)
...
- Для anti-storm обработки ошибок в `state_data` сохраняются поля `*_last_error_at`, `*_error_count`, `*_next_retry_at`; worker применяет экспоненциальный backoff и пропускает повторные попытки до `next_retry_at`. После успешной отправки nudge эти поля очищаются.
