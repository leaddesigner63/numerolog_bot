...
- Если Gemini возвращает ошибки авторизации/лимита или модель не найдена (например, 400/404), бот сначала перебирает оставшиеся ключи Gemini, а затем автоматически переключается на OpenAI.
...
- Запросы к LLM идут в Gemini как к основному провайдеру, при ошибках 401/403/404/429/5xx/timeout выполняется перебор ключей, затем fallback на ChatGPT с ограниченными ретраями (до 2 на Gemini и до 1 на ChatGPT). При наличии нескольких API-ключей каждого провайдера выполняется автоматический перебор ключей до fallback.
...
- Генерация отчётов использует LLM-маршрутизатор: Gemini (основной) с 1–2 ретраями на 5xx/timeout,
  перебором ключей при ошибках 401/403/404/429/5xx/timeout и fallback на ChatGPT (у ChatGPT — 1 retry на transient).
