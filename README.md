...
4. Экран оплаты для T1/T2/T3 теперь финальный: он показывается только после завершения данных (и анкеты для T2/T3), непосредственно перед генерацией отчёта (S3 -> S6 -> S7). На S3 отображаются название тарифа, цена и кнопка оплаты. Заказ и платёжная ссылка создаются только на этом финальном шаге checkout. Для T0 поведение не изменено.
