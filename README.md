...
Бот отправляет сообщение напрямую в Telegram администраторам из `ADMIN_IDS` и сохраняет его
в `feedback_messages` со статусом `sent`/`failed`. Отправка в группу отключена.
