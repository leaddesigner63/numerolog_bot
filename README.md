...
- массовая загрузка LLM-ключей из файла: каждая непустая строка — отдельный ключ, провайдер выбирается в интерфейсе и применяется ко всем строкам (переносы строк CRLF/LF обрабатываются автоматически), значения сохраняются как есть;
...
- Пустые/пробельные значения API-ключей не используются при обращении к LLM и автоматически пропускаются, чтобы не блокировать генерацию.
...
- Если в базе нет ключей для конкретного провайдера (`gemini` или `openai`), сервис пытается использовать ключи с пустым провайдером (например, если ключи были загружены через админку без выбора провайдера).
...
- Если итоговый список LLM-ключей пуст, сервис пишет INFO-лог с провайдером, количеством ключей из БД и окружения, признаком SQLite-фолбэка и значением `DATABASE_URL` (или `fallback_sqlite=true`) для быстрой диагностики конфигурации.
