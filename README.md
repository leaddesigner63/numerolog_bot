...
2. Для T1–T3 подтвердите оплату, затем заполните экран **Мои данные** (дата рождения — `DD:MM:YYYY`, время рождения — `HH:MM`).
3. Для T2/T3 пройдите расширенную анкету — ответы сохраняются после каждого шага и доступны для продолжения.
4. После завершения анкеты (или сразу после профиля для T0/T1) получите отчёт — PDF формируется автоматически и отправляется в чат, а кнопка «Выгрузить PDF» остаётся для повторной загрузки.
...
- На экране S3 всегда показывается ссылка на оферту (если задана).
