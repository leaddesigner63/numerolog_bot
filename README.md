...
- Кнопка “Я оплатил(а)” запрашивает статус у платёжного провайдера и не запускает ввод данных/генерацию,
  пока заказ не получит статус `paid` из webhook.
...
- Статус `paid` проставляется **только** после webhook; ручная проверка статуса не меняет оплату.
