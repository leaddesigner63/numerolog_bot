...
Пошаговая инструкция по автодеплою находится в `docs/AUTODEPLOY.md`. Workflow: `.github/workflows/deploy.yml`.


## SEO-матрица и автоматические проверки лендинга

- Матрица ключей хранится в `web/keyword_matrix_100.csv` (ровно 100 строк без учёта заголовка).
- Автопроверка ограничений выполняется скриптом `scripts/check_keyword_matrix.py`:
  - не более 1 ключа на абзац целевых страниц;
  - один и тот же ключ не чаще 2 раз на всём сайте;
  - отсутствие ключей на `web/legal/*`;
  - обязательная формулировка для кластера «натальная карта»: «альтернативный формат: нумерология + ИИ» со ссылкой на `/faq/`.
- Скрипт подключён в общий прогон `scripts/test.sh` и CI.
