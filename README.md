...
- Для обложки используется отдельный слой `tX_bg_cover.webp` (если файл существует, он становится страницей 1), далее внутренние страницы продолжают использовать `tX_bg_main.webp`.
- Для иконки обложки можно добавить `tX_icon_cover.png`; если её нет, рендер берёт `tX_icon_main.png`.
