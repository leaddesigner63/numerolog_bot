...
Для доступа нужен ключ `ADMIN_API_KEY` из `.env`: при открытии страницы появится экран ввода ключа, после чего админка откроется и сохранит ключ в cookie и localStorage для последующих запросов.  
