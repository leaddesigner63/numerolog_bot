...
- массовая загрузка LLM-ключей из файла: каждая строка — отдельный ключ, поля через `|` в порядке `provider|key|priority|is_active`, значения сохраняются как есть, пустые поля допустимы;
