...
Для доступа нужен ключ `ADMIN_API_KEY` из `.env` — введите его в интерфейсе (ключ сохраняется в localStorage, если доступно, и применяет обновление данных сразу после сохранения).  
Если задан `ADMIN_ALLOWED_IPS`, доступ разрешается только указанным IP-адресам (например, публичному IP вашей админской сети или IP вашего reverse-proxy). Админка учитывает заголовки `X-Forwarded-For`/`X-Real-IP` (и стандартный `Forwarded`) — убедитесь, что ваш proxy передаёт их корректно.  
