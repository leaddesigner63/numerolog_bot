...
- На экранах S2/S3 всегда показывается кнопка “Возвратов нет” и ссылка на оферту (если задана).
...
- Если bucket или его зависимости недоступны (например, отсутствует `boto3` или переменные окружения), бот отправит PDF без сохранения и не упадёт — в таком случае повторная выдача приведёт к повторной генерации.
...
- Если ключи LLM не настроены (нет `GEMINI_API_KEY` и `OPENAI_API_KEY`), бот сообщает об этом и не запускает генерацию.
...
- `GEMINI_API_KEY`/`OPENAI_API_KEY` — ключи LLM (если оба отсутствуют, генерация отчёта блокируется и показывается предупреждение).
