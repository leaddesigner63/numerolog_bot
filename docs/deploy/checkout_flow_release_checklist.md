...
5. Проверить worker после релиза:
   - heartbeat воркера свежий (обновляется в ожидаемом интервале),
   - очередь report jobs не растёт аномально (нет устойчивого накопления `pending`/`in_progress`).
