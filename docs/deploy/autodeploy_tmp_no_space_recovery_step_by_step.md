...
- Перед выбором каталога выполняется пробный `mktemp -d`; если в каталоге нельзя создать временную папку (например, раздел переполнен), кандидат автоматически пропускается.
...
- Workflow передаёт `TMPDIR` на удалённый хост, чтобы дочерние скрипты использовали тот же каталог, и тоже проверяет кандидаты через пробный `mktemp`.
