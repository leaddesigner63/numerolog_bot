...
### Какие поля подписываются
В текущей реализации поддерживаются **несколько вариантов** подписи:
1. HMAC-SHA256 от **сырых байтов HTTP body** (`raw_body`) с секретом.
2. MD5 от строки `token + secret` (если в payload присутствует `token`).

### Порядок полей
- Для HMAC-SHA256 порядок полей берётся как есть из сырых байтов тела запроса (канонизация полей в коде не выполняется).
- Для MD5-варианта порядок полей payload не используется; применяется только конкатенация `token` затем `secret`.
