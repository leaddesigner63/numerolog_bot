...
def screen_s4(state: dict[str, Any]) -> ScreenContent:
    ...
    if has_profile:
        ...
        text = _with_screen_prefix(
            "S4",
            (
                "В превью-отчёте ты увидишь, на что ИИ обратил внимание в первую очередь: ключевые сильные стороны, возможные зоны роста и формат полного анализа. Коротко и по делу."
                " Начни с малого — а дальше решать тебе. Краткий мини отчёт (~30 % полного): несколько сильных сторон, возможные зоны роста и аккуратная ретроспектива, чтобы оценить подход."
...
def screen_s5(state: dict[str, Any]) -> ScreenContent:
    ...
    text = _with_screen_prefix(
        "S5",
        "Для получения максимально качественного результата требуется заполнить анкету. ✍️\n"
        "p.s. Если хочешь, чтобы разбор получился максимально крутым — распиши каждый пункт анкеты подробнее. "
        "Не стесняйся и не экономь на деталях: ограничений по длине нет.💥\n\n"
        f"{progress_line}"
    ).strip()
