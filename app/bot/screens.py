...
def screen_s0(_: dict[str, Any]) -> ScreenContent:
    text = _with_screen_prefix(
        "S0",
        "Шаг 1. Коротко о том, как работает разбор.\n"
        ...
        "Нажмите «Далее», чтобы выбрать тариф."
    )
    rows = [
        [
            InlineKeyboardButton(
...
def screen_s1(_: dict[str, Any]) -> ScreenContent:
    text = _with_screen_prefix(
        "S1",
        "Шаг 2. Выбор тарифа для вашего сценария.\n"
        ...
        "Нажмите на тариф, который хотите получить сейчас."
    )
    rows = [
        [
            InlineKeyboardButton(
...
def screen_s3(state: dict[str, Any]) -> ScreenContent:
    ...
    if payment_processing_notice:
        ...
        text_parts.append(
            "Шаг 3. Подтверждение оплаты выбранного тарифа.\n"
...
def screen_s4(state: dict[str, Any]) -> ScreenContent:
    ...
    if has_profile:
        ...
        text = _with_screen_prefix(
            "S4",
            (
                "Шаг 4. Заполните данные для бесплатного превью-отчёта.\n"
                ...
                "Нажмите «Дальше», чтобы начать ввод данных."
            ),
        )
    else:
        text = _with_screen_prefix(
            ...
            (
                f"{payment_success_banner}"
                "Шаг 4. Заполните профиль для генерации отчёта.\n"
                ...
                "Нажмите «Заполнить данные», чтобы продолжить."
            ),
        )
...
def screen_s5(state: dict[str, Any]) -> ScreenContent:
    ...
    text = _with_screen_prefix(
        "S5",
        "Шаг 5. Дополнительная анкета для углублённого отчёта.\n"
        "• Ответы уточняют контекст и усиливают персональные выводы.\n"
        "• Подробные формулировки улучшают качество рекомендаций.\n"
        f"• {progress_line or 'Прогресс: 0/0.'}\n"
        "Нажмите кнопку ниже, чтобы заполнить или продолжить анкету."
    ).strip()
